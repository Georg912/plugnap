...
object Schedule {
    ...
    private fun startMinute(date: LocalDate, p: Prefs) =
        if (p.weekendEnabled && isWeekendNight(date)) p.weekendStart else p.windowStart

    private fun endMinute(date: LocalDate, p: Prefs) =
    ...
    fun windowFor(date: LocalDate, p: Prefs): Window {
    ...
    fun currentWindow(now: LocalDateTime, p: Prefs): Window? =
        listOf(now.toLocalDate().minusDays(1), now.toLocalDate())
            .map { windowFor(it, p) }
            .firstOrNull { !now.isBefore(it.start) && now.isBefore(it.end) }

    fun inWindow(now: LocalDateTime, p: Prefs): Boolean =
        p.allDay || currentWindow(now, p) != null

    fun nextStart(now: LocalDateTime, p: Prefs): LocalDateTime {
        for (i in 0..7L) {
    ...
    fun nextEnd(now: LocalDateTime, p: Prefs): LocalDateTime {
        for (i in -1..7L) {
            val w = windowFor(now.toLocalDate().plusDays(i), p)
    ...
    fun skipUntilMillis(p: Prefs): Long =
